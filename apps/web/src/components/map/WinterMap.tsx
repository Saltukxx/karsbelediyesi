...
import {
  ...
  TAZELIK_RENK,
  rotaTazelik,
  type WinterDriverDto,
  type WinterMaterialDto,
  type WinterRouteDto,
  type WinterVehicleDto,
...
export default function WinterMap({
  routes,
  vehicles,
  drivers,
  materials,
  ...
  // Tazelik hesabı render anına sabitlenir (SSR/CSR tutarlılığı önemli değil, client-only bileşen)
  ...
  function startEditRoute(route: WinterRouteDto) {
  ...
  async function submitOperation(formData: FormData) {
    await kisOperasyonKaydet(formData);
  ...
  function deleteRoute(id: string) {
    ...
    fd.set("id", id);
    startTransition(() => kisRotaSil(fd));
  }

  function toggleAktif(route: WinterRouteDto) {
    const fd = new FormData();
    fd.set("id", route.id);
  ...
  function deleteOperation(id: string) {
    if (!window.confirm("Operasyon kaydı (varsa tuz düşümüyle birlikte) silinsin mi?")) return;
    const fd = new FormData();
    fd.set("id", id);
    startTransition(() => kisOperasyonSil(fd));
  }
    ...
    <div className="flex flex-col gap-4 lg:flex-row">
      <div className="kb-map relative min-h-[480px] flex-1 overflow-hidden rounded-xl border border-kb-border shadow-md">
        ...
        >
          ...
          {routes
            ...
            .map((r) => {
              const tazelik = rotaTazelik(r.sonOperasyon, now);
              ...
              return (
                <Polyline
                  ...
                  pathOptions={{
                    color: r.aktif ? TAZELIK_RENK[tazelik] : "#94a3b8",
                ...
                >
                  <Tooltip sticky>
                    {r.ad} — öncelik {r.oncelik}
                    {r.sonOperasyon
                      ? ` — son işlem ${format(new Date(r.sonOperasyon), "dd.MM HH:mm")}`
                      : " — hiç işlem görmedi"}
                  </Tooltip>
                  <Popup maxWidth={320}>
                    <div className="space-y-1.5 text-sm">
                      <p className="font-semibold">
                        ...
                        <span className="text-xs font-normal text-gray-500">
                          ({ROTA_TIP_LABELS[r.tip]}, öncelik {r.oncelik}
                          {r.aktif ? "" : ", pasif"})
                        </span>
                      </p>
                            ...
                            {r.sonOperasyonlar.map((o) => (
                              <li key={o.id} className="flex items-center gap-2 text-xs">
                                <span>
                                  ...
                                  {OPERASYON_TIP_LABELS[o.tip]}
                                  {o.tuzKg != null ? ` (${o.tuzKg} kg tuz)` : ""}
                                  {o.arac ? ` · ${o.arac}` : ""}
                                </span>
                                {canEdit && (
                                  <button
      ...
      <aside className="w-full shrink-0 space-y-4 lg:w-96">
            ...
            <form ref={opFormRef} action={submitOperation} className="space-y-3">
              ...
              <div>
                ...
                <select name="tip" defaultValue="KARMA" className={inputCls}>
                  {Object.entries(OPERASYON_TIP_LABELS).map(([k, v]) => (
              ...
              <div>
                <label className={labelCls}>Şoför / Operatör</label>
              ...
              <div>
                <label className={labelCls}>Kullanılan tuz (kg)</label>
                <input
                  name="tuzKg"
                  type="number"
                  step="0.01"
                  min="0"
                  onChange={(e) => setTuzGirildi(e.target.value.trim() !== "")}
                  className={inputCls}
                />
              </div>
              {tuzGirildi && (
                <div>
                  <label className={labelCls}>Tuz malzemesi (stoktan düşülür) *</label>
                  <select name="tuzMaterialId" required className={inputCls}>
                    <option value="">— Seçiniz —</option>
                    {materials.map((m) => (
                      <option key={m.id} value={m.id}>
                        {m.kod} — {m.ad} (stok: {m.stok} {m.birim})
                      </option>
                    ))}
                  </select>
                </div>
              ...
              >
                ...
                <div>
                  ...
                  <input
                    ...
                    className={inputCls}
                    placeholder="Örn. Cumhuriyet Cad. ana arter"
                  />
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <select
                      name="tip"
                      defaultValue={editingRoute?.tip ?? "KARMA"}
                      className={inputCls}
                    >
                      {Object.entries(ROTA_TIP_LABELS).map(([k, v]) => (
                        <option key={k} value={k}>
                          {v}
                        </option>
                      ))}
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className={inputCls}
                    >
                      <option value="1">1 — Ana arter</option>
                ...
                <li key={r.id}>
                  ...
                  >
                    <span
                      ...
                      style={{
                        background: r.aktif ? TAZELIK_RENK[tazelik] : "#94a3b8",
                      }}
                    />
                    <span className="min-w-0 flex-1 truncate">
                      <span className="font-medium">{r.ad}</span>{" "}
                      <span className="text-xs text-kb-muted">Ö{r.oncelik}</span>
                    </span>
                    <span className="shrink-0 text-xs text-kb-muted">
                      {r.aktif
                        ? r.sonOperasyon
                          ? TAZELIK_LABEL[tazelik]
                          : "işlem yok"
                        : "pasif"}
                    </span>
                  </button>
        ...
        <div className="space-y-1.5 rounded-lg border border-kb-border bg-kb-surface-raised p-4 text-xs text-kb-muted shadow-sm">
          ...
          <p>
            <span className="mr-1 inline-block h-2 w-4 rounded-sm align-middle" style={{ background: TAZELIK_RENK.taze }} />{" "}
            Son 4 saatte işlem gördü
          </p>
          <p>
            <span className="mr-1 inline-block h-2 w-4 rounded-sm align-middle" style={{ background: TAZELIK_RENK.orta }} />{" "}
            4–12 saat önce
          </p>
          <p>
            <span className="mr-1 inline-block h-2 w-4 rounded-sm align-middle" style={{ background: TAZELIK_RENK.eski }} />{" "}
            12 saatten eski / hiç
          </p>
          <p>
          ...
          <p>Çizgi kalınlığı = öncelik (kalın = ana arter).</p>
        </div>
      </aside>
    </div>
  );
