...
export default function CleaningMap({
  ...
  function startEditRoute(route: CleaningRouteDto) {
  ...
  async function submitRoute(formData: FormData) {
    ...
    if (editRouteId) {
      ...
      id = await temizlikRotaGuncelle(formData);
    } else {
      id = await temizlikRotaKaydet(formData);
    }
  ...
  function toggleAktif(route: CleaningRouteDto) {
    const fd = new FormData();
    fd.set("id", route.id);
    fd.set("aktif", route.aktif ? "false" : "true");
    startTransition(() => {
      void temizlikRotaGuncelle(fd);
    });
  }

  function focusRoute(r: CleaningRouteDto) {
                  ...
                  <Popup maxWidth={320}>
                    <div className="space-y-1.5 text-sm">
                      ...
                      <p className="text-xs text-gray-600">
                        ...
                        {r.sonGorev
                          ? ` — son görev ${format(new Date(r.sonGorev), "dd.MM HH:mm")}`
                          : " — henüz görev yok"}
                      </p>
              ...
              >
                ...
                <div>
                  ...
                  <input
                    ...
                    className={inputCls}
                    placeholder="Örn. Cumhuriyet Cad. süpürme hattı"
                  />
                </div>
                <div>
              ...
              <li key={r.id}>
                ...
                >
                  <span
                    ...
                    style={{ background: r.aktif ? "#0d9488" : "#94a3b8" }}
                  />
                  ...
                  <span className="shrink-0 text-xs text-kb-muted">
                    {r.aktif
                      ? r.sonGorev
                        ? format(new Date(r.sonGorev), "dd.MM")
                        : "görev yok"
                      : "pasif"}
                  </span>
                </button>
